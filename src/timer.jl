...
gatetime(::Val{:Sym}, c::Union{ControlBlock, Daggered, PrimitiveBlock}) where N = Sym(Symbol(:T,occupied_locs(c) |> length))
gatetime(s::Val{:Sym}, c::Union{Add, Scale}) = throw(MethodError(gatetime, (s, c)))
gatetime(::Val{:Sym}, c::Measure) = c.collapseto isa Nothing ? Sym(:Tm) : Sym(:Tm) + Sym(:Treset)
